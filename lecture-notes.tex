...
\begin{center}
	\begin{tikzcd}
	X\arrow[r,"f"] & Y\arrow[r,"g"]&\arrow[dr,"\alpha"]Cf\arrow[r]& \Sigma X\arrow[r] &\Sigma Y\arrow[r] &\dots\\
	& &  & C_g \arrow[ur,"\beta"]\arrow[u]
	\end{tikzcd}
...
We just need to make sure that this vertical component of the diagram commutes. We can get $C_g$ from collapsing the cone $CY$ in $Cf$. Now we need a map $C_g\wedge T_\ast\to \Sigma X$ What is it on $CX$ and what is it on $CY$? $h(x,s,t)=(f(x),t-st)$ What is it on $CY$? $h(y,s,t)=(y,s+t-st)$. It crushes down the part of $Y$ outside of $X$. Ultimately, we're trying to say that if we think about the sequence \begin{center}
	\begin{tikzcd}
	X\arrow[r,"f"] &Y \arrow[r,"g_0"] & Cf\arrow[r]\arrow[d,equal] &\Sigma X\arrow[r] & \Sigma Y\arrow[r] & \Sigma Cf\arrow[r] & \dots\\
	X\arrow[r,"f"] & Y \arrow[r,"g_0"]& Cf\arrow[r,"g_1"] & Cg_0\arrow[u] \arrow[r] & Cg_1\arrow[r] &\dots
	\end{tikzcd}
\end{center}
...
Now we will resume talking about fibrations. A cofiber is a map such that the cofiber and the homotopy cofiber are the same. The slogan is that the fiber and the homotopy fiber are the same, where the fiber of a map from $X$ to $Y$ is specified by the pullback\begin{center}
	...
	& \ast\arrow[d]\\
	X\arrow[r,"f"] & Y
	\end{tikzcd} which gives \begin{tikzcd}
	f^{-1}(\ast)\arrow[r]\arrow[d]& \ast\arrow[d]\\
	X\arrow[r,"f"] & Y
	\end{tikzcd}
	as the pullback
\end{center}
And additionally \begin{center}
	\begin{tikzcd}
	Ff \arrow[r] \arrow[d]&py \arrow[d] \\ X\arrow[r,"f"] &Y
	\end{tikzcd}
\end{center}
...
\begin{definition}
	$f:E\to B$ is a fibration if there exists a lift by \begin{center}
		\begin{tikzcd}
		Y\arrow[r] \arrow[d] & E \arrow[d,"f"]\\
		Y\times I\arrow[r] &B
		\end{tikzcd} with the same data as \begin{tikzcd}
		E\arrow[dd] & & \arrow[ll] E^I \arrow[dd]\\
		& Y \arrow[ul] \arrow[dr]\arrow[ur,dashrightarrow]& \\
		B && \arrow[ll] B^I
		\end{tikzcd}
	\end{center}
\end{definition}
$(F,G)$ is an adjoint pair \begin{center}
	\begin{tikzcd}
	FX\arrow[r]\arrow[d] &  Y\arrow[d] \\
	FX'\arrow[r] & Y'
	\end{tikzcd}
	\begin{tikzcd}
	X\arrow[r]\arrow[d] &  GX\arrow[d] \\
	X'\arrow[r] & GY'
	\end{tikzcd}
\end{center}
...
If \begin{center}
	\begin{tikzcd}
	E'\arrow[r] \arrow[d,"q"]&E\arrow[d,"p"]\\ B'\arrow[r]& B' 
	\end{tikzcd}
\end{center}
...
\begin{center}
	\begin{tikzcd}
	X\times_f PY\arrow[r]\arrow[d] & PY\arrow[d]\\X\arrow[r,"f"]& Y
	\end{tikzcd}
...
As a proposition if $f:E\to B$ is a fibration, then the fiber and the homotopy fiber are equivalent. The proof of that is the duel of the proof we gave for the mapping cone and the honest quotient. Given a fibration we can form the sequence \begin{center}
	\begin{tikzcd}
	\Omega Ff\arrow[r] & \Omega X\arrow[r] &\Omega Y \arrow[r] & Ff\arrow[r]& X\arrow[r,"f"] &Y
	\end{tikzcd}
\end{center}
...
\begin{center}
	\begin{tikzcd}
	& &\Sigma\Omega Ff\arrow[d] \arrow[r] & \Sigma\Omega X\arrow[r] \arrow[d]&\Sigma\Omega Y\arrow[r] \arrow[d]&\Sigma\Omega Ff\arrow[r] \arrow[d]&\Sigma X\arrow[d,equal]\\
	\dots \arrow[r] & \Omega Y\arrow[r] \arrow[d,equal]&F f \arrow[r] \arrow[d]&X\arrow[r]\arrow[d]& Y \arrow[r] \arrow[d]&Cf\arrow[r] \arrow[d]&\Sigma X\arrow[r]&\dots \\
	& \Omega Y\arrow[r]& \Omega Cf\arrow[r] &\Omega\Sigma X\arrow[r] & \Omega\Sigma Y\arrow[r] &\Omega\Sigma Cf
	\end{tikzcd}
\end{center}
...
And suppose we have the commuting diagram\begin{center}
	\begin{tikzcd}
	A\arrow[r]\arrow[d,rightarrowtail] & X\arrow[d,twoheadrightarrow]\\
	B\arrow[r]\arrow[ur,dashrightarrow] &Y
	\end{tikzcd}
\end{center}
...
\begin{center}
	\begin{tikzcd}
	\emptyset\arrow[d]\arrow[r] &X\arrow[d]\\Z\arrow[ur,dashrightarrow]\arrow[r] &Y
	\end{tikzcd} is surjective and 
	\begin{tikzcd}
	Z\times \partial I \arrow[r] \arrow[d]& X\arrow[d] Y\\
	Z\arrow[ur]\times I\arrow[r] &Y
	\end{tikzcd} is injective
\end{center}
...
\section{October 27th, 2021}
	...
	\begin{center}
		\begin{tikzcd}
		0\arrow[r] &\text{Ext}^1(H_{p-1})\arrow[r] & H^p(X;G)\arrow[r] & \text{Hom}(H_p(X),G)\arrow[r] & 0
		\end{tikzcd}
	\end{center}
	...
	\begin{center}
		\begin{tikzcd}
		0\arrow[r] & G\arrow[r] & I_0\arrow[r] & I_1\arrow[r] &0
		\end{tikzcd}
	\end{center}
	...
	\begin{center}
		\begin{tikzcd}
		0\arrow[r] & \text{Hom}(A,G)\arrow[r] & \text{Hom}(A,I_0)\arrow[r]& \text{Hom}(A,I_1)\arrow[r] & \text{Ext}(A,G)\arrow[r] &0
		\end{tikzcd}
	\end{center}
	...
	\begin{center}
		\begin{tikzcd}
		\dots \arrow[r] & H^{p-1}(X;I_0)\arrow[r] & H^{p-1}(X;I_1)\arrow[r] & H^p(X;G)\arrow[r] &H^{p}(X;I_0)\arrow[r] & H^{p}(X;I_1)\arrow[r] & \dots
		\end{tikzcd}
	\end{center}
	...
	\begin{center}
		\begin{tikzcd}
		&\text{Hom}(B_p,I)\arrow[r,rightarrowtail] & C^{p+1} X\\
		0 & \arrow[l]\text{Hom}(Z_p,I) \arrow[u]&\arrow[l,twoheadrightarrow] C^p X  \arrow[u]& \arrow[l,rightarrowtail] \text{Hom}(B_p,I)& \arrow[l] 0 \\
		&\text{Hom}(H_pX,I) \arrow[u,rightarrowtail]& C^{p-1} X\arrow[r,twoheadrightarrow] \arrow[u]& \text{Hom}(Z_{p-1}, I)\arrow[u,twoheadrightarrow] \\
		& 0\arrow[u]
		\end{tikzcd}
	\end{center}
	...
	Next, we need to get injective resolutions. There is a theorem that says that $R$-mod ``has enough injectives." Professor Blumberg thinks this is from Grothendieck.  For any module $M$ there exists an injective $i$ and an injection $0\to M\to I$. We can make anything sit inside an injective module. We can ake cokernels in the sequence \begin{center}
		\begin{tikzcd}
		0\arrow[r] & M\arrow[r] & I_0\arrow[r] 7 i-1\arrow[r] & I_2\arrow[r] &\dots
		\end{tikzcd}
		and embed then in the module. then, when $R$ is a PID, this terminates after two terms. after $I_1$. More explicitly, in $\mathbb{Z}$ we will call a group divisible if for all $g\in G$ there exists and $n$ and $Y$ such that $ny=g$. Divisible groups are injective. This tells us what to do for finitely generated $\mathbb{Z}$ modules, as we get torsion pieces and free pieces. For the free piece we have \begin{center}
			\begin{tikzcd}
			0\arrow[r] & \mathbb{Z}\arrow[r] & \mathbb{Q}\arrow[r] &\mathbb{Q}/\mathbb{Z}\arrow[r] & 0 
			\end{tikzcd}
		\end{center}
	...
	The last fact we needed was uniqueness up to chain homotopy of injective resolutions. Let us sketch this. The real theorem is that if we have \begin{center}
		\begin{tikzcd}
		0\arrow[r] & M \arrow[r] &I_0\arrow[r]& I_1\arrow[r]& I_2\arrow[r] &\dots
		\end{tikzcd}
	\end{center}
	...
	\begin{center}
		\begin{tikzcd}
		0\arrow[r] & M \arrow[r] \arrow[d,equal] &N_0\arrow[r]\arrow[d,"f_0",dashrightarrow]& N_1\arrow[r]& N_2\arrow[r] &\dots\\
		0\arrow[r] & M \arrow[r] &I_0\arrow[r]& I_1\arrow[r]& I_2\arrow[r] &\dots
		\end{tikzcd}
	\end{center}
	...
	\begin{center}
		\begin{tikzcd}
		0\arrow[r] & M \arrow[r] \arrow[d,equal] &N_0\arrow[r]\arrow[d,"f_0",dashrightarrow]& \text{im}(N_0\to N_1)\arrow[r]& N_1\arrow[r] &\dots\\
		0\arrow[r] & M \arrow[r] &I_0\arrow[rr] && I_1
		\end{tikzcd}
	\end{center}
	...
	We know things about this product. They ultimately boil down to facts about the diagonal map. For instance, we know that if we have $X$ and we have \begin{center}
		\begin{tikzcd}
		X\arrow[r,"\delta"]\arrow[d,"\Delta"] & X\times X\arrow[d,"\text{id}\times \Delta"]\\
		X\times X \arrow[r,"\text{id}\times \Delta"] &X\times X\times X
		\end{tikzcd}
		\end{center}
...
We can also think of this in terms of bundles (covering space). \begin{center}
	\begin{tikzcd}
	\coprod_{x\in X} H_n(X,X-\{x\}) (**)\arrow[d]\\
	X
	\end{tikzcd} where we topologize this to make it an honest bundle/covering space. 
\end{center}
...
\begin{proof}
	\begin{center}
		\begin{tikzcd}
		0 \arrow[r] &H_n(A\mid A\cup B)\arrow[r]\arrow[d,"\cong"]& H_n(M\mid A)\oplus H_n(M\mid B)\arrow[r] \arrow[d,"\cong"]& H_n(M\mid A\cap B)\arrow[d,dashrightarrow,"\cong"]\\
